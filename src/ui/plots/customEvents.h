...
public:
    ...
    void setTooltipTimeout(const int ms) { m_tooltipTimeout = ms; }
...
protected:
    ...
    struct Intercerp {
        qreal distance{};
        QPointF pos;
        QPointF IPpixel; // Added to store pixel position calculated in background
        bool isValid = false;
    };

    QFutureWatcher<Intercerp> *watcher;
    ...
    virtual Intercerp findIntersection(const QPointF &chartPos, const QPointF &mousePos, const QVector<qreal> &limits);
...
protected:
    Intercerp findIntersection(const QPointF &chartPos, const QPointF &mousePos, const QVector<qreal> &limits) override;
